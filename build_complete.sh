...
print_help() {
  ...
  echo ""
  echo "Artix-specific options:"
  echo "  When distro=artix, the 'release' argument selects the init system:"
  echo "    openrc (default), runit, s6, dinit"
  echo "  Example: sudo ./build_complete.sh dedede distro=artix"
  echo "  Example: sudo ./build_complete.sh dedede distro=artix release=runit desktop=kde"
}
...
if [ ! "$rootfs_dir" ]; then
  ...
  if [ "$distro" = "debian" ]; then
    ...
    if [ "$release" != "openrc" ] && [ "$release" != "runit" ] && [ "$release" != "s6" ] && [ "$release" != "dinit" ]; then
